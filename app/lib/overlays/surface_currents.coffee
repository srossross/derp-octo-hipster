...
class SurfaceCurrentsOverlay extends SurfaceCurrentsOverlayBase
    ...
    bounds_changed: =>
        ###
        Called when the map bounds are changed.
        This will reset the canvas's parent div to the origin.
        ###
        if @pause
            return
        ...
        @div_.style.left = swp.x + 'px';
        @div_.style.top = nep.y + 'px';
